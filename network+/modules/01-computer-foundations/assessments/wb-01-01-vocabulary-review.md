---
worksheet_type: vocabulary-review
number: "1.1.E"
title: "Vocabulary Review"
module: 01
lesson: 01
unit: 1
objective_ids: ["on-ramp"]
points: 20
est_minutes: 12
answer_key: instructor/answer-keys/wb-01-01-answers.md
---

<!-- NetworkAcademy+ printable worksheet · follows docs/WORKBOOK-SYSTEM.md · print B&W, portrait -->

# NetworkAcademy+ · Lab Manual
### Worksheet 1.1.E — Vocabulary Review &nbsp;&nbsp;|&nbsp;&nbsp; Points: ______ / 20

**Name:** ____________________________ **Date:** ____________ **Time spent:** ______ min
**Module 01 · Lesson 01 · Unit 1** &nbsp;|&nbsp; **Objective:** vocabulary

---

**Part A — Define it.** Write the meaning of each term in your own words (one sentence).

| Term | Your definition |
|------|-----------------|
| **Computer** | _______________________________________________ |
| **Input** | _______________________________________________ |
| **Output** | _______________________________________________ |
| **Program** | _______________________________________________ |

**Part B — Name it.** Read the meaning; write the term.

1. The work a computer does by following instructions. → ____________________
2. The physical parts of a computer you can touch. → ____________________
3. Information you put into a computer. → ____________________

**Part C — Use it.** Write one sentence that uses the term correctly.

- **Output:** ________________________________________________________
- **Program:** _______________________________________________________

**Part D — Spaced recall.** Answer from memory, without peeking back.

- What are the **two goals** of this whole course? _______________________
- Name **one "hidden" computer** from this lesson (something that doesn't look like a computer). _______________________

---

Self-score: ______ / 20 &nbsp;&nbsp; Band (circle): ✅ 🟡 🟠 🔴 &nbsp;&nbsp; Verified by: ____________ &nbsp; ☐ Redo
_NetworkAcademy+ · Worksheet 1.1.E · CompTIA Network+ N10-009 · Page ___ of ____
